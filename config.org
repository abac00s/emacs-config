...
#+BEGIN_SRC emacs-lisp
  ...
  (dolist (mode lisp-modes)
    (add-hook (mode-to-hook mode)
	      (lambda ()
		(enable-paredit-mode)
		(define-key evil-normal-state-local-map "L" #'paredit-forward)
		(define-key evil-normal-state-local-map "H" #'paredit-backward)
		(define-key evil-normal-state-local-map "J" #'paredit-forward-down)
		(define-key evil-normal-state-local-map "K" #'paredit-backward-up))))
#+END_SRC
*** Scheme
#+BEGIN_SRC emacs-lisp
  (require 'geiser)
